...
export function initialHomeSearch(params) {
  const count=(key,fallback,min,max)=>{const raw=params.get(key);const n=Number(raw);return raw!==null && /^\d+$/.test(raw) && n>=min && n<=max ? n : fallback;};
  const children = count('children',0,0,3);
  return {destination:destinationKey({countryCode:params.get('countryCode') || params.get('country') || '',code:params.get('destinationCode') || ''}),
    ...
    adults:count(params.has('adults')?'adults':'people',2,1,6),children,rooms:1,
    childrenAges:Array.from({length:children},(_,i)=>params.get('childrenAges')?.split(',').slice(0,3)[i] ?? '')};
}
...
export function buildHomeSearch(form, destinations, diagnostic=false, now=new Date()) {
  ...
  if (form.rooms !== undefined && form.rooms !== 1) errors.guests='Сейчас доступен поиск одного номера. Укажите 1 номер.';
