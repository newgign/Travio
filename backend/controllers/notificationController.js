...
async function sendTest(req, res) {
  try {
    ...
    return res.status(500).json({ message: error.message || "Ошибка тестового уведомления" });
  }
}
...
async function retry(req, res) {
  try {
    ...
    return res.status(500).json({ message: error.message || "Ошибка повторной отправки" });
  }
}
